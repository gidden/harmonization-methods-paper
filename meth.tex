...
offset of base year values (i.e., $h(t_i) - m(t_i)$). \code{aneris} implements a
...
$\beta$, scales linearly from 1 to 0 in $[t_i, t_f)$ and is shown in Equation
  \ref{eqs:factor}. Note that region, species, and sector indices have been
  dropped for clarity. Each equation is a function of time, model trajectory,
  historical trajectory, harmonization time step, and a convergence time step
  ($t_f$), at which point the harmonized trajectory converges to the
  unharmonized trajectory. \code{aneris} provides a number of methods to choose
