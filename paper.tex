...
\subsection{\code{aneris} Workflow and Python Implementation}\label{sec:workflow}

We herein present \code{aneris}' conceptual workflow as well as specific details
of the Python implementation. The workflow is depicted in Figure
\ref{fig:workflow}, and the implementation is depicted in Figure
\ref{fig:software}. 

\code{aneris} is comprised of a number of conceptual workflow and software
components shown graphically in Figures \ref{fig:workflow} and
\ref{fig:software}, respectively. Unharmonized model data and a run-control
configuration are read in via an Excel spreadsheet. Data is assumed to be in the
IAMC format, i.e., using \code{Model}, \code{Scenario}, \code{Region},
\code{Variable}, and \code{Unit} columns in addition to columns representing
each modeled time period.
