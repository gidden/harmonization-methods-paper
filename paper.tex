...
%%% Acronyms
\newacronym{ssps}{SSPs}{Shared Socioeconomic Pathways}
...
workflow. The implementation is depicted in Figure \ref{fig:software}, and the
conceptual workflow is depicted in Figure \ref{fig:workflow}. The software is
composed of three primary components: the \texttt{HarmonizationDriver},
\texttt{Harmonizer}, and data processing routines (shown in red, green, and
blue, respectively in Figure \ref{fig:software-workflow}).

The \texttt{HarmonizationDriver} is an object designed to interface with
user-provided data and configuration files. Input data is assumed to be an Excel
file the in the in the IAMC format, i.e., using \code{Model}, \code{Scenario},
\code{Region}, \code{Variable}, and \code{Unit} columns in addition to columns
representing each modeled time period, which is the standard format within the
IAM community. It is responsible for downselecting data into unique combinations
for each model and scenario and then invoking the harmonization
process. Finally, it adds any exogenous emissions trajectories as required by
the user and recompiles the results and writes them to disc. The
\texttt{HarmonizationDriver} acts the primary interface for high-level users as
shown in Listing \ref{lst:tutorial}. Furthermore, a \gls{cli} is provided to
allow users to more easily incorporate the harmonization process in scripted
workflows (Listing \ref{lst:cli}, Figure \ref{fig:software}). 
...
The \texttt{Harmonizer} is a Python class whose concern is to harmonize various
model value trajectories given historical data and possible user method
\textit{overrides}, i.e., non-default methods. It is used by the
\texttt{HarmonizationDriver}; however it is also available to the user as a
first-class object. The \texttt{Harmonizer} requires that input data conform to
the \code{aneris} calculation data format, which expliciltly sepearates the
emissions species from the sector contributing the emissions (these are combined
in the single \code{Variable} column in the IAMC format). Because the
\texttt{Harmonizer} is configure for a single instance of a model and scenario,
the canonical data format includes columns named \texttt{region},
\texttt{sector}, \texttt{gas}, and \texttt{units}. Once configured with
appropriate input data (model and history) as well as potential method
...
used to combine historical emissions to native model regions (where historical
data is defined at a higher spatial resolution than a model; see, e.g., Figure
\ref{fig:regions}) and a \texttt{FormatTranslator} class which defines an
interface for translating \texttt{pandas.DataFrame}s between the IAMC format
expected for input and output data and the calculation format used by
...
\begin{figure}
  ...
  \subfloat[Workflow\label{fig:workflow}]{
    ...
    \caption{ Panel \ref{fig:software} shows the various objects in the
      \code{aneris} codebase as well as a short description of their scope of
      ...
      configurations are shown in purple. Operations goverened by the
...
source. In such cases, override methods can ameliorate the issues associated
with the default method choice. 

In order to help identify these cases, harmonization \textit{diagnostics} are
provided which analyze the relative difference between harmonized and
unharmonized trajectories at their mid and end-points. These values are
configurable by the user, but defaults of $400$\% and $200$\%, respectively, are
provided based on experiences of using \code{aneris} to date. If override
methods are provided, they are used instead of the default methods as determined
by the default method decision tree. Furthermore, users can set the
above-mentioned thresholds as well as the LUC method used in the decision tree
(see Figure \ref{fig:decision_tree}).
