...
\gls{luluc} are known to have high year-to-year variation, and therefore
...
The \texttt{Harmonizer} is a class whose concern is to harmonize model value
...
by cleaning input data. Cleaning operations include adding (0-valued) model
...
scenarios represent two bounding cases in the use of a harmonization approach
...
mapped directly to the 5-region definition used in the \glspl{rcp}. Historical
...
each scenario, and therefore 1940 trajectories were harmonized in total. \noxx
...
regional trajectories for both the reference and mitigation scenarios closely
matching the unharmonized model behavior and also result in global aggregates
that agree with the unharmonized trajectory over the modeled time period.
...
trajectory with overrides more accurately match the original unharmonized model
